    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .firstTextBaseline) {
                ...
                Text("Últimos 7 días · Claude Code").font(.ui(12)).foregroundStyle(palette.muted)
            ...
            VStack(alignment: .leading, spacing: 5) {
                ForEach(Array(legend.enumerated()), id: \.offset) { index, model in
                    LegendRow(model: model,
                              color: palette.modelColor(index),
...
private struct LimitTile: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(title).font(.ui(11, .medium)).foregroundStyle(palette.muted).lineLimit(1)
            Text("\(Int(bar.percent.rounded()))%")
                .font(.ui(22, .bold))
                .monospacedDigit()
                .foregroundStyle(palette.color(forPercent: bar.percent))
                .lineLimit(1)
                .minimumScaleFactor(0.7)
            UsageBar(fraction: bar.fraction, color: palette.color(forPercent: bar.percent), height: 4)
            Text(Fmt.countdown(to: bar.resetsAt, now: now))
                .font(.mono(10)).foregroundStyle(palette.muted).lineLimit(1).minimumScaleFactor(0.8)
        }
        .padding(.horizontal, 11)
        .padding(.vertical, 8)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(RoundedRectangle(cornerRadius: 12).fill(palette.elevated))
    }
}
...
private struct LegendRow: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(model.name)
                .font(.ui(11, .medium)).foregroundStyle(palette.text)
            ...
            Text(Fmt.tokens(model.tokens))
                .font(.ui(11)).monospacedDigit().foregroundStyle(palette.muted)
...
private struct FooterStat: View {
    ...
    var body: some View {
        VStack(spacing: 1) {
            Text(value).font(.ui(14, .bold)).monospacedDigit().foregroundStyle(palette.text)
                .lineLimit(1).minimumScaleFactor(0.7)
            Text(label).font(.ui(10)).foregroundStyle(palette.muted)
                .lineLimit(1).minimumScaleFactor(0.8)
        }
        .frame(maxWidth: .infinity)
        .overlay(alignment: .leading) {
            if divider { Rectangle().fill(palette.line).frame(width: 1) }
        }
    }
}
